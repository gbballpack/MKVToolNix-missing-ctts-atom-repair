...
Files=$(find . -name "*.x265-VXT.mp4" -type f)
for f in $Files
do
  ...
  echo "Converting files to MKV and merging subtitle into the new file"
    for i in **/*.x265-VXT.mkv; do
        ffmpeg -i "$i" -y -flags +global_header -movflags faststart -c:v copy -c:a copy "${i%.x265-VXT.mkv}.x265-VXT.mp4";
    done
  echo $?

  # Deletes the new/temporary MKV that matches an MP4 file with the same name
  echo "Deleting MKV files that match MP4 files"
    for f in **/*.x265-VXT.mkv; do
