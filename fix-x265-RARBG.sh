...
Files=$(find . -name "*.x265-RARBG.mp4" -type f)
for f in $Files
do
  ...
  echo "Converting mkv files into mp4 and overiding the original mp4 file"
    for i in **/*.x265-RARBG.mkv; do
        ffmpeg -i "$i" -y -flags +global_header -movflags faststart -c:v copy -c:a copy "${i%.x265-RARBG.mkv}.x265-RARBG.mp4";
    done
  echo $?

  # Deletes the new/temporary MKV that matches an MP4 file with the same name
  echo "Deleting MKV files that match MP4 files"
    for f in **/*.x265-RARBG.mkv; do
